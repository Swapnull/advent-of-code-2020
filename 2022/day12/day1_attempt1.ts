...
const getNextStep = (curr, visited) => {
  // console.log("--- curr ---", curr);

  const newVisit = [...visited, curr];
  const [x, y] = curr;

  if (newVisit.length === 2) console.log(curr);

  if (grid[x][y] === "E") {
    console.log("PATH COMPLETE", newVisit.length - 1);
    return newVisit.length;
  }
